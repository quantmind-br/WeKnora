...
The FAQ knowledge base is used to maintain standard Q&A such as return policies, reimbursement procedures, and common troubleshooting. Each entry contains a standard question, similar questions, negative questions, and answers; retrieval matches against questions and returns answers according to the hit strategy.

After creating an FAQ-type knowledge base, you can bulk-import Q&A pairs via Excel or CSV, then add similar questions and negative questions. An FAQ knowledge base can be retrieved by agents alongside document knowledge bases; when FAQ priority is enabled and the direct-answer threshold is met, the standard answer can be returned directly.
...
Entry management, import, and retrieval configuration are covered below; the deduplication and synchronization mechanisms are in the reference sections.

## Creating and Maintaining Q&A

After creating an FAQ knowledge base, fill in a standard question and an answer for each entry. Similar questions cover different phrasings, and negative questions exclude questions that are easily mismatched. An entry can have multiple answers, and you choose whether to return all of them or one at random.

Entries can be managed by category; you can disable individual Q&A pairs you're not using for now, or prevent entries from appearing in recommendations. FAQ categories are single-tag, and are managed separately from the multi-tag mechanism of document knowledge bases.

## Import and Export

Use the template to prepare standard questions, similar questions, negative questions, and answers, with multi-value fields separated by `##`. When importing, choose append or replace: append matches entries by content hash and merges duplicates; replace deletes the old entries and keeps only the content of this import batch. When you need to check the data first, use the API's `dry_run` to run validation only.

Import results distinguish success, failure, partial failure, and skipped, and include the failure reasons. CSV and JSON exports can be used for backup, editing, and re-import.

## Checking Hit Results

FAQ retrieval considers vectors, keywords, and tag priority together, and entries with a negative question that exactly matches the query are excluded. Results include the matched question, score, and match type, which you can use to tune similar questions, negative questions, and thresholds.

## Import and Retrieval Reference

### Bulk Import {#_4-bulk-import}
...
type FAQBatchUpsertPayload struct {
    ...
    TaskID      string            `json:"task_id"` // Optional, auto-generated if not provided; custom values may only contain [A-Za-z0-9_-], ≤128 characters
...
### Retrieval Hit Strategy {#_6-retrieval-hit-strategy}
...
4. **Iterative recall** (`applyFAQPostProcessing`): when the number of unique entries after filtering is below `match_count` and the vector results are maxed out, `iterativeRetrieveWithDeduplication` is triggered — up to 5 iterations, doubling TopK each time (both the seed `MatchCount*3` and each round's growth are capped at 500, stopping once the cap is hit), with deduplication and negative-match filtering caches, terminating early if no new results appear;
...
## Entry and API Reference

### Data Model {#_1-data-model}

#### Storage Form: FAQ Entry = One Chunk {#_1-1-storage-form-faq-entry-one-chunk}

Each FAQ entry corresponds to one `Chunk` record (`chunk_type = "faq"`), belonging to the `Knowledge` of type `faq` in the knowledge base (this Knowledge is automatically created the first time an entry is created). The entry's structured content is stored in `Chunk.Metadata` (JSON):
...
Common fields reused on Chunk: `SeqID` (auto-incrementing integer, the entry ID exposed in the external API), `TagID` (category tag, default tag name constant `UntaggedTagName = "Untagged"`), `IsEnabled` (disable switch), `Flags` (bit0 `ChunkFlagRecommended` — whether it can be recommended), `ContentHash` (deduplication hash, see [Normalization and Content Hashing (Deduplication Core)](#_3-normalization-and-content-hashing-deduplication-core)).

#### API Projection: FAQEntry {#_1-2-api-projection-faqentry}
...
#### KB-Level FAQ Configuration (FAQConfig) {#_1-3-kb-level-faq-configuration-faqconfig}
...
### API Endpoints {#_2-api-endpoints}

`internal/handler/faq.go` (routes registered in `internal/router/routes_knowledge.go`; KB access control is the same as for the knowledge base: reads go through KBAccessRead, writes through KBAccessWrite; API Keys require `ingest` / `retrieve` capability):

| Method | Path | Function |
| --- | --- | --- |
| GET | `/knowledge-bases/:id/faq/entries` | List entries (pagination / tag / keyword / enabled status) |
...
List query parameters: `page` / `page_size`, `tag_id` (tag seq_id, compatible with the legacy single tag) or `tag_ids` (tag UUIDs, comma-separated, OR semantics), `keyword` + `search_field` (`standard_question` / `similar_questions` / `answers`, searches all fields if omitted), `sort_order` (`asc`; by default sorted by update time, descending), `is_enabled` (`true` / `false` filters by enabled status; returns everything if omitted).
...
### Normalization and Content Hashing (Deduplication Core) {#_3-normalization-and-content-hashing-deduplication-core}
...
### Differences from Regular Knowledge (Documents) {#_5-differences-from-regular-knowledge-documents}
...
| Retrieval post-processing | Negative-match filtering + iterative recall (see [Retrieval Hit Strategy](#_6-retrieval-hit-strategy)) | Standard fusion reranking |
...
### Clone / Sharing Synchronization Mechanism {#_7-clone-sharing-synchronization-mechanism}
