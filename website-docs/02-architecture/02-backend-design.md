...
The Go backend organizes request handling into Handler, Service, Repository and infrastructure layers, with dependencies assembled via uber/dig. `cmd/server` handles startup and shutdown, while `internal/` implements routing and authorization, business services, and storage access.

## Layered Architecture {#_1-layered-architecture}
...
| Access Control | `internal/application/access/` | Gin-independent shared authorization rules: three-tier knowledge base access resolution, organization sharing permissions, knowledge base scope of shared Agents, file and message artifact authorization, cross-KB clone/move authorization |
...
## Dependency Injection: internal/container (uber/dig) {#_2-dependency-injection-internal-container-uber-dig}
...
### dig Features Used {#_2-1-dig-features-used}
...
### Registration Order and Conditional Assembly {#_2-2-registration-order-and-conditional-assembly}
...
The concurrency of the 6 Asynq worker pools can be adjusted via system settings / environment variables (defaults: Core=8, PostProcess=2, Enrichment=12, Maintenance=4, Shared=6, Wiki=8, `WEKNORA_ASYNQ_*_CONCURRENCY`); the queue topology is defined in `internal/types/task.go` (default, chat_attachment, postprocess, summary, multimodal, graph, question, memory, sync, low/maintenance, wiki, etc., including auto-tagging and memory extraction).

### Resource Cleanup and Factories {#_2-3-resource-cleanup-and-factories}
...
## cmd/server Startup Flow {#_3-cmd-server-startup-flow}
...
- **Bootstrap failures do not block startup**: `bootstrap.go` records failures with `logger.Warnf`. System admin bootstrap only takes effect when the deployment has no system admin yet, and a restart does not restore permissions that have been revoked;
...
## Route Organization and RBAC Assembly (internal/router) {#_4-route-organization-and-rbac-assembly-internal-router}

### NewRouter Assembly Order {#_4-1-newrouter-assembly-order}
...
1. `gin.New()` + `MultipartFormCleanup` (deletes the temporary files that multipart parsing wrote to disk once the request finishes) + `SetTrustedProxies` (`WEKNORA_TRUSTED_PROXIES`, by default trusts only loopback and private network ranges, preventing forged `X-Forwarded-For` headers from bypassing per-IP rate limiting);
...
5. Public routes registered **before authentication**: IM platform callbacks (`/api/v1/im`, each platform has its own signature verification), Web Embed public routes (`/api/v1/embed/:channel_id`, `middleware.EmbedAuth` publish-token authentication + Redis rate limiting), the built-in MCP Server (`/mcp/:endpoint_id`, `middleware.MCPEndpointAuth` per-endpoint Bearer Token authentication), short-lived capability URLs (resource grants), sandbox terminal/desktop WebSockets (browsers cannot send auth headers during the handshake, so a short-lived ticket obtained through an authenticated POST is used instead), and local browser extension connections (`/api/v1/local-browser/*`);
...
7. `v1 := r.Group("/api/v1")`: first `v1.Use(rbacGuards.apiKeyAuthorizer.Middleware())` (API Key gateway — JWT sessions pass straight through), then calls 30-plus `RegisterXxxRoutes(v1, handler, rbacGuards)` in sequence;
...
### Route Group Overview {#_4-2-route-group-overview}
...
| `/sandbox-configs`, `/me/env-vars`, `/me/browser` | RegisterSandboxConfigRoutes / RegisterMyEnvVarRoutes etc. | `full_access`; `/me/*` only for the JWT user themselves |
...
### rbacGuards: Centralized Permission Matrix {#_4-3-rbacguards-centralized-permission-matrix}
...
## Middleware List (internal/middleware) {#_5-middleware-list-internal-middleware}

In the order requests pass through them:

| Middleware | File | Responsibility and Key Logic |
| --- | --- | --- |
| `MultipartFormCleanup()` | multipart_cleanup.go | After the request finishes (including failures and panic recovery), deletes the files the multipart form wrote to the system temp directory, preventing the container's `/tmp` from growing indefinitely |
| `cors.New` (gin-contrib) | router.go | Allows `Authorization`, `X-API-Key`, `X-Tenant-ID`, `X-Embed-Session`, `X-WeKnora-Desktop-Token`, plus the `MCP-Protocol-Version`/`Mcp-Session-Id` headers required by MCP Streamable HTTP, and other headers; MaxAge 12h |
...
| `Language()` | language.go | Determines the document processing language: `WEKNORA_LANGUAGE` env var > first tag in `Accept-Language` > default `en-US` |
...
| `MCPEndpointAuth(...)` | mcp_endpoint_auth.go | Mounted only on `/mcp/:endpoint_id`: validates the endpoint Bearer Token, injects the endpoint's owning space, the machine principal, and an API Key-style permission scope derived from the tool allowlist and knowledge base scope |
| `AttachAuthenticatedUser(...)` | ws_auth.go | For WebSocket routes such as the sandbox terminal: uses the user resolved from the short-lived ticket to establish the same authenticated session as `Auth`; membership and role are still resolved from the database |
...
| `RequireKBAccess(resolver, perm, ...)` | kb_access.go | Adapts the three-tier KB access resolution from `internal/application/access` (owned → organization-shared → shared-agent read-only), and **rewrites** the `TenantIDContextKey` in `Request.Context()` to the KB's source tenant, so downstream retrieval automatically lands on the correct tenant's data |
...
## Domain Model Overview (internal/types) {#_6-domain-model-overview-internal-types}
...
erDiagram
    ...
    MODEL {
        ...
        string source "local / remote / vendor identifier"
...
## Error Handling Conventions (internal/errors) {#_7-error-handling-conventions-internal-errors}
...
## Logging System (internal/logger) {#_8-logging-system-internal-logger}
...
- **LLM debug logging** (`llm_logger.go`): enabled when `LLM_DEBUG_LOG=true`, logging every LLM call per request_id into a separate file (`LLMCallRecord`: CallType Chat/Embedding/Rerank/VLM, model, duration, full messages and tool calls, errors), auto-cleaned after 7 days; used for troubleshooting prompt and context issues.

## Key Utility Libraries (internal/common, internal/utils) {#_9-key-utility-libraries-internal-common-internal-utils}
...
| `utils/presign.go` | `SignFileURL` / `VerifyFileURLSig`, `SystemHMACKey` | HMAC-SHA256 presigned file URLs (default 2h, used for IM embedded images); the signing key is taken from `SYSTEM_SIGNING_KEY`, falling back to `SYSTEM_AES_KEY` when unset |
