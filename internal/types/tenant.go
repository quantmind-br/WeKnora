...
type ParserEngineConfig struct {
	...
	// MinerU self-hosted parsing parameters. The protocol is detected from the server: 4.0+ uses the V1 API, earlier versions use /file_parse.
	MinerUServerAPIKey string `json:"mineru_server_api_key,omitempty"` // 4.0+: server --api-key
	MinerUTier         string `json:"mineru_tier,omitempty"`           // 4.0+: flash/basic/standard/advanced; empty uses the server default
	// The backend, vLLM, formula/table, and language parameters below only apply to 3.x and earlier.
