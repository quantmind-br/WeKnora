...
// SafeJoinUnderBase joins a caller-supplied relative suffix under baseDir and returns
// an absolute path that still lies within baseDir. Leading and trailing separators are
// stripped so inputs such as "/etc" cannot override the root; ".." that still points to
// a parent after path.Clean is rejected. An empty suffix means baseDir itself.
