...
func TestVolcengineClientOutboundShape(t *testing.T) {
	...
	got, err := client.Rerank(context.Background(), "Is conversation data retained?", []string{"It is retained", "It is not retained"})
	require.NoError(t, err)
	require.Len(t, got, 2)
	assert.Equal(t, 0, got[0].Index)
	assert.Equal(t, "It is retained", got[0].Text)
	...
	assert.Equal(t, "Is conversation data retained?", request.Datas[0].Query)
	require.NotNil(t, request.Datas[0].Content)
	assert.Equal(t, "It is retained", *request.Datas[0].Content)
