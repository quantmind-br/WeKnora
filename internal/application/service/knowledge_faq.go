...
func (s *knowledgeService) ListFAQEntries(ctx context.Context,
	...
	if len(tagUUIDs) == 0 && legacyTagSeqID > 0 {
		...
		if err != nil {
			return nil, werrors.NewNotFoundError("Tag not found")
		}
...
func (s *knowledgeService) CreateFAQEntry(ctx context.Context,
	...
	// Concurrent creation of the same standard question must be serialized: the duplicate check below can only see entries already persisted to the database,
	// it can't catch sibling requests still being indexed (upstream timeout retries can produce this kind of concurrency).
	...
	// Check whether the standard question and similar questions duplicate other entries
	...
	// Set SeqID if an ID is specified (used for data migration)
	...
	// Index chunk: set a hard cap on the interactive-creation indexing step to avoid embedding jitter dragging out the request
	...
	cancelIndex()
	if indexErr != nil {
		// If indexing fails, delete the already-created chunk. A failed rollback leaves behind a chunk in stored status
		// Leftover: it doesn't show up in the list, but it gets hit by repeated validation, so it must trigger an alert rather than fail silently.
	...
	// Convert to FAQEntry and return
	entry, err := s.chunkToFAQEntry(chunk, kb, tagSeqIDMap)
	if err != nil {
		return nil, err
	}
...
func (s *knowledgeService) GetFAQEntry(ctx context.Context,
	...
	if entrySeqID <= 0 {
		return nil, werrors.NewBadRequestError("Entry ID cannot be empty")
	}
	...
	if err != nil {
		return nil, werrors.NewNotFoundError("FAQ entry not found")
	}

	// Verify the chunk belongs to the current knowledge base
	if chunk.KnowledgeBaseID != kb.ID || chunk.TenantID != tenantID {
		return nil, werrors.NewNotFoundError("FAQ entry not found")
	}

	// Verify it is of type FAQ
...
func (s *knowledgeService) UpdateFAQEntry(ctx context.Context,
	...
	if chunk.KnowledgeBaseID != kb.ID {
		return nil, werrors.NewForbiddenError("No permission to operate on this FAQ entry")
	}
	if chunk.ChunkType != types.ChunkTypeFAQ {
		return nil, werrors.NewBadRequestError("Only FAQ entries can be updated")
	}
	meta, err := sanitizeFAQEntryPayload(payload)
	if err != nil {
		return nil, err
	...
	// Get the old list of similar questions, for incremental update
	...
	if existing, err := chunk.FAQMetadata(); err == nil && existing != nil {
		...
		// Save the old content for incremental comparison
	...
	// Incremental indexing optimization: only index content that changed
	if questionIndexMode == types.FAQQuestionIndexModeSeparate && len(oldSimilarQuestions) > 0 {
		// Incremental update in separate indexing mode
		...
		// Combined mode or first creation, use full indexing
		// Incremental delete: only delete the index for removed similar questions
		...
		// Use needDelete=false, since EFPutDocument automatically overwrites documents with the same SourceID
...
func (s *knowledgeService) AddSimilarQuestions(ctx context.Context,
	...
	if len(questions) == 0 {
		return nil, werrors.NewBadRequestError("Similar questions list cannot be empty")
	}
	...
	if err != nil || meta == nil {
		return nil, werrors.NewBadRequestError("Failed to get FAQ metadata")
	}
...
func (s *knowledgeService) UpdateFAQEntryStatus(ctx context.Context,
	...
	if chunk.KnowledgeBaseID != kb.ID || chunk.ChunkType != types.ChunkTypeFAQ {
		return werrors.NewBadRequestError("Only FAQ entries can be updated")
	}
...
func (s *knowledgeService) UpdateFAQEntryFieldsBatch(ctx context.Context,
	...
	if len(req.ByTag) > 0 {
		for tagSeqID, update := range req.ByTag {
			...
			if err != nil {
				return werrors.NewNotFoundError(fmt.Sprintf("Tag %d not found", tagSeqID))
			}
			...
			if update.TagID != nil {
				if *update.TagID > 0 {
					...
					if err != nil {
						return werrors.NewNotFoundError(fmt.Sprintf("Tag %d not found", *update.TagID))
					}
...
func (s *knowledgeService) UpdateFAQEntryTag(ctx context.Context, kbID string, entryID string, tagID *string) error {
	...
	if chunk.KnowledgeBaseID != kb.ID || chunk.ChunkType != types.ChunkTypeFAQ {
		return werrors.NewBadRequestError("Only FAQ entry tags can be updated")
	}
...
func (s *knowledgeService) UpdateFAQEntryTagBatch(ctx context.Context, kbID string, updates map[int64]*int64) error {
	...
	if len(tagSeqIDSet) > 0 {
		...
		for _, tag := range tags {
			if tag.KnowledgeBaseID != kb.ID {
				return werrors.NewBadRequestError(fmt.Sprintf("Tag %d does not belong to the current knowledge base", tag.SeqID))
			}
	...
	for entrySeqID, tagSeqID := range updates {
		...
		if tagSeqID != nil && *tagSeqID > 0 {
			...
			if !ok {
				return werrors.NewBadRequestError(fmt.Sprintf("Tag %d not found", *tagSeqID))
			}
			resolvedTagID = tag.ID
		}
...
func (s *knowledgeService) SearchFAQEntries(ctx context.Context,
	...
	// Batch query TagName and append it to the results
	if len(entries) > 0 {
		// Collect all TagIDs (seq_id) that need to be queried
		...
		if len(tagSeqIDs) > 0 {
			...
			if err != nil {
				...
				// Build a TagSeqID-to-TagName mapping
...
func (s *knowledgeService) DeleteFAQEntries(ctx context.Context,
	...
	if len(entrySeqIDs) == 0 {
		return werrors.NewBadRequestError("Please select the FAQ entries to delete")
	}
	kb, err := s.validateFAQKnowledgeBase(ctx, kbID)
	if err != nil {
		return err
	...
	for _, seqID := range entrySeqIDs {
		...
		if err != nil {
			return werrors.NewNotFoundError("FAQ entry not found")
		}
		if chunk.KnowledgeBaseID != kb.ID || chunk.ChunkType != types.ChunkTypeFAQ {
			return werrors.NewBadRequestError("Contains invalid FAQ entries")
		}
...
// Category (required), Question (required), Similar questions (optional - separate multiple with ##), Negative examples (optional - separate multiple with ##),
// Bot answers (required - separate multiple with ##), Reply all (optional - default FALSE), Disabled (optional - default FALSE),
// Disable recommendations (optional - default False, recommendable)
...
// ExportFAQEntriesJSON exports all entries under the FAQ knowledge base as a JSON array,
// with fields compatible with FAQEntryPayload, to support an "export → edit → re-append import" loop.
...
func (s *knowledgeService) buildFAQCSV(chunks []*types.Chunk, tagMap map[string]string) []byte {
	...
	headers := []string{
		"Category (required)",
		"Question (required)",
		"Similar questions (optional - separate multiple with ##)",
		"Negative examples (optional - separate multiple with ##)",
		"Bot answers (required - separate multiple with ##)",
		"Reply all (optional - default FALSE)",
	...
	for _, chunk := range chunks {
		...
		row := []string{
			...
			boolToCSV(!chunk.IsEnabled),                                 // Whether disabled: negated
			boolToCSV(!chunk.Flags.HasFlag(types.ChunkFlagRecommended)), // Whether recommendation is blocked: negated
		}
...
func (s *knowledgeService) validateFAQKnowledgeBase(ctx context.Context, kbID string) (*types.KnowledgeBase, error) {
	...
	if kb.Type != types.KnowledgeBaseTypeFAQ {
		return nil, werrors.NewBadRequestError("This operation is only supported for FAQ knowledge bases")
	}
	return kb, nil
}
...
func (s *knowledgeService) ensureFAQKnowledge(
	...
	knowledge := &types.Knowledge{
		...
		Description:      "FAQ entry container",
...
func buildFAQChunkContent(meta *types.FAQChunkMetadata, mode types.FAQIndexMode) string {
	...
	// Negative examples should not be included in Content, since they should not be indexed
	// Whether the answer is included depends on the index mode
...
// checkFAQQuestionDuplicate checks whether the standard question and similar questions duplicate other entries in the knowledge base
// excludeChunkID is used to exclude the entry currently being edited (used during updates)
// Follow the same check approach as batch import: build the set of existing questions first, then check uniformly
func (s *knowledgeService) checkFAQQuestionDuplicate(
	...
	// First check whether the current entry's own similar questions duplicate the standard question
	for _, q := range meta.SimilarQuestions {
		if q == meta.StandardQuestion {
			return werrors.NewBadRequestError(fmt.Sprintf("Similar question 「%s」 cannot be the same as the standard question", q))
		}
	}

	// Check whether the current entry's own similar questions duplicate one another
	seen := make(map[string]struct{})
	for _, q := range meta.SimilarQuestions {
		if _, exists := seen[q]; exists {
			return werrors.NewBadRequestError(fmt.Sprintf("Duplicate similar question 「%s」", q))
		}
		seen[q] = struct{}{}
	}

	// Check whether the counter-example question duplicates the standard question or similar questions (counter-examples must not equal positive examples)
	...
	for _, q := range meta.NegativeQuestions {
		...
		// Check whether the counter-example duplicates the standard question
		if q == meta.StandardQuestion {
			return werrors.NewBadRequestError(fmt.Sprintf("Negative example question 「%s」 cannot be the same as the standard question", q))
		}
		// Check whether the counter-example duplicates similar questions
		if _, exists := positiveQuestions[q]; exists {
			return werrors.NewBadRequestError(fmt.Sprintf("Negative example question 「%s」 cannot be the same as a similar question", q))
		}
		// Check whether counter-examples duplicate one another
		if _, exists := negativeQuestionsSeen[q]; exists {
			return werrors.NewBadRequestError(fmt.Sprintf("Duplicate negative example question 「%s」", q))
		}
		negativeQuestionsSeen[q] = struct{}{}
	}

	// Merge the standard question and all similar questions, and check for conflicts with other entries using a single DB query (replacing a full scan)
	...
	if err != nil || existingMeta == nil {
		return werrors.NewBadRequestError("Standard or similar question duplicates an existing entry")
	}

	// 5–7. Same error semantics as the original full scan: check the standard question first, then check each similar question in turn
	...
	if meta.StandardQuestion != "" {
		if meta.StandardQuestion == existingMeta.StandardQuestion {
			return werrors.NewBadRequestError(fmt.Sprintf("Standard question 「%s」 already exists", meta.StandardQuestion))
		}
	...
	for _, q := range meta.SimilarQuestions {
		...
		if q == existingMeta.StandardQuestion {
			return werrors.NewBadRequestError(fmt.Sprintf("Similar question 「%s」 already exists", q))
		}
...
// buildFAQTagResolver preloads the tag UUID mapping referenced by entries, avoiding per-entry resolveTagID calls.
...
// hashQuestion computes a hash of the question content, used to generate a stable sourceID.
...
// If no tag is specified, creates or finds the "Uncategorized" tag
...
func (s *knowledgeService) resolveTagID(ctx context.Context, kbID string, payload *types.FAQEntryPayload) (string, error) {
	...
	// If tag_id (seq_id) is provided, use tag_id first
	if payload.TagID != 0 {
	...
	// If tag_name is provided, look up or create the tag
	if payload.TagName != "" {
	...
	// If neither is provided, use the "Uncategorized" tag
...
func sanitizeFAQEntryPayload(payload *types.FAQEntryPayload) (*types.FAQChunkMetadata, error) {
	// Handle AnswerStrategy, defaulting to all
	answerStrategy := types.AnswerStrategyAll
	if payload.AnswerStrategy != nil && *payload.AnswerStrategy != "" {
		...
		default:
			return nil, werrors.NewBadRequestError("answer_strategy must be 'all' or 'random'")
		}
	}
	meta := &types.FAQChunkMetadata{
	...
	meta.Normalize()
	if meta.StandardQuestion == "" {
		return nil, werrors.NewBadRequestError("Standard question cannot be empty")
	}
	if len(meta.Answers) == 0 {
		return nil, werrors.NewBadRequestError("Provide at least one answer")
	}
	return meta, nil
}
...
// buildFAQIndexInfoList builds the FAQ index info list, supporting separate indexing mode
func (s *knowledgeService) buildFAQIndexInfoList(
	...
	// If in combined indexing mode, use the original logic
	...
	// Separate indexing mode: create an independent index item for each question
	indexInfoList := make([]*types.IndexInfo, 0)

	// Standard question index item
	...
	// Create one index item per similar question
