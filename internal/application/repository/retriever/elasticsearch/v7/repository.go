...
// EstimateStorageSize estimates the storage space size
...
// Compute the storage footprint of a single index (in bytes)
func (e *elasticsearchRepository) calculateStorageSize(embedding *elasticsearchRetriever.VectorEmbedding) int64 {
	...
	if embedding.Embedding != nil {
		// 4 bytes/dimension (full-precision float)
		vectorSizeBytes = int64(len(embedding.Embedding) * 4)
	}

	// 3. Metadata size (fixed overhead such as ID, timestamp, etc.)
	metadataSizeBytes := int64(250) // About 250 bytes of metadata

	// 4. Index overhead (ES index amplification factor is approximately 1.5)
