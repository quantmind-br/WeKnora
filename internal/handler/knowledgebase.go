...
// @Description  Low-level recall: hybrid vector + keyword search, with no rerank by default (enable it with the rerank field); use /knowledge-search for general retrieval. POST is recommended; GET with a JSON body is still supported for legacy clients.
// @Tags         Knowledge Base
// @Accept       json
// @Produce      json
// @Param        id             path      string             true   "Knowledge Base ID"
// @Param        request        body      types.SearchParams true   "Search parameters"
// @Param        resource_urls  query     string  false  "File reference form; public returns directly loadable links"  Enums(handle, public)  default(handle)
