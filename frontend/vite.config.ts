...
export default defineConfig({
  ...
  server: {
    port: 5173,
    host: true,
    // Proxy configuration for the development environment
    proxy: {
      '/mcp/': {
  ...
  // `vite preview` serves the production build output (dist) locally - the closest environment to a release image:
  // Same minification / chunk splitting / CSS load order, exposing issues that only appear in production builds
  // (e.g. theme variables being overridden by bundle order). Usage: npm run build && npm run preview
  preview: {
    port: 4173,
    host: true,
    proxy: {
