...
export async function testMCPService(id: string): Promise<MCPTestResult> {
  ...
  // Backend response format: { success: true, data: MCPTestResult }
  // The response interceptor already returns data, so response is { success: true, data: {...} }
  if (response && response.data) {
    return response.data
  }
  // If the format is wrong, try returning response directly (it might be the raw returned data)
  return response
}
