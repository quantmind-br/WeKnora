        ...
        <div
          ...
          <template v-for="(agent, index) in filteredAgents"
            ...
            <div v-show="!isAgentRowHidden(agent)" class="agent-card" :class="{
              ...
              <!-- Row-end favorite action, kept separate from the more menu. -->
...
// The "Workspace" view always keeps "built-in → created by me → created by colleagues"; the user's sort choice only affects order within each group.
...
// The space view always keeps "shared by me → editable → view only"; the user's sort choice only affects order within each group.
...
// Split shared resources into editable and read-only groups based on their actual permission.
...
// Shared card sizing and interactions.
.agent-card {
  .resource-card();
